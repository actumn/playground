...
struct KeyWord {				// 어휘와 종별의 대응을 관리
	const char *keyName;		// if나 for 등을 관리
...
ifstream fin;				// 입력 스트림
#define MAX_LINE 2000		// 최대 프로그램 행 수
...
void nextLine()				// 다음 한 줄을 가져온다
{
	...
	if (fin.eof()) {				// 파일 종료
		fin.clear(); fin.close();	// clear는 재오픈에 대비
		endOfFile_F = true;
		return;
	}
...
Token nextTkn()				// 다음 토큰
{
	...
	if (CH == '\0') return Token(EofLine);		// 행 끝
	...
	case Digit:										// 수치 상수
		...
		return Token(kd, txt, atof(txt.c_str()));	// IntNum도 dobule 형으로 저장
	case DblQ:
		NEXT_CH();
