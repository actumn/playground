...
void convert_to_internalCode(char *fname)	// 코드 변환
{
	...
	// main 함수가 있으면 메인 함수를 호출 코드를 설정
	set_startPc(1);							// 1행부터 시작
	if (mainTblNbr != -1) {
...
void convert_rest()							// 문의 나머지 처리
{
	...
	for (;;) {
		...
		switch (token.kind) {				// 아래의 키워드가 도중에 나타나는 일은 없다
...
void varDecl()							// var를 사용하는 변수 선언
{
	...
	for (;;) {
		...
		enter(tmpTb, varId);						// 변수 등록 (주소도 등록)
...
void set_aryLen()						// 배열 크기 설정
{
	...
	tmpTb.aryLen = (int)token.dblVal + 1;			// var a[5]는 첨자 0~5가 유효이므로 +1
...
void fncDecl()							// 함수 정의
{
	...
	Gtable[fncTblNbr].dtTyp = DBL_T;				// 함수형은 double로 고정ㅇ
	...
	if (token.kind != ')') {						// 인수 있음
		for (;; token = nextTkn()) {
			...
			setCode(Lvar, tblNbr);					// 인수는 Lvar로 처리
			++Gtable[fncTblNbr].args;				// 인수 갯수를 +1
			if (token.kind != ',') break;			// 선언 종료
			setCode(',');
		}
	}
...
void backPatch(int line, int n)			// 함수 정의
{
...
void push_intercode()					// 변환한 내부 코드를 저장
{
	...
	*codebuf_p++ = '\0';
	...
	catch (bad_alloc) { err_exit("메모리를 확보 할 수 없습니다."); }
	codebuf_p = codebuf;				// 다음 처리를 대비해 저장할 곳 맨 앞으로 위치지정
}

bool is_localScope()					// 함수 내 처리 중이면 참
{
	return fncDecl_F;
}
