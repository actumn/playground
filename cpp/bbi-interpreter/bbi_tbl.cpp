...
int enter(SymTbl& tb, SymKind kind)		// 심볼 테이블 등록
{
	...
	// 확인
	mem_size = tb.aryLen;
	if (mem_size == 0) mem_size = 1;									// 단수 변수일 때
	...
	// 주소 설정
	if (kind == fncId) tb.adrs = get_lineNo();							// 함수의 시작 행
	else {
...
int searchName(const string& s, int mode)	// 이름 검색
{
	...
	case 'V':		// 변수명 검색
		...
		if (is_localScope()) return searchName(s, 'L');		// 로컬 영역 처리 중
		else return searchName(s, 'G');						// 글로벌 영역 처리 중
	}
	return -1;		// 발견되지 않음
}
