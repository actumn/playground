...
CodeSet code;							// 코드 세트
int startPc;							// 실행 시작 행
int Pc = -1;							// 프로그램 카운터 -1: 실행 중 아님
int baseReg;							// 베이스 레지스터
int spReg;								// 스택 포인터
int maxLine;							// 프로그램 끝 행
...
public:
	...
	double pop() {										// 읽기 & 삭제 (원래의 pop과는 다르니 주의)
...
void syntaxChk()						// 구문 검사
{
	...
	for (Pc=1; Pc < (int)intercode.size(); Pc++) {
		...
		case If: case Elif: case While:
			code = nextCode(); (void)get_expression(0, EofLine);	// 식 값
			break;
		case For:
			code = nextCode();
		...
		case Return:
			code = nextCode();
			if (code.kind != '?' && code.kind != EofLine) (void)get_expression();	// 반환 값
...
void set_startPc(int n)					// 시작 행 설정
{
	startPc = n;
}
...
void statement()						// 문
{
	...
	if (code.kind == If) end_line = endline_of_If(Pc);	// if 문 일 때의 끝
	...
	case While:
		...
		Pc = end_line + 1;
		break;
	case For:											// for 제어 변수, 초깃값, 최종 값, 증분식
		save = nextCode();
		...
		endDt = get_expression(To, 0);					// 최종 값을 보존
														// 증분값을 보존
	...
	case Break:
		code = nextCode();
		post_if_set(break_Flg);							// ?가 있으면 처리
		if (!break_Flg)
			++Pc;
		break;
	case Exit:
...
void expression(int kind1, int kind2)			// 조건부 식 처리
{
...
void term(int n)								// n은 우선순위
{
	...
	term(n+1);
	while (n == opOrder(code.kind)) {					// 우선순위가 같은 연산자가 계속된다
		op = code.kind;
...
void factor()									// 인자
{
	...
	case Not: case Minus: case Plus:
		...
		if (kd == Not) stk.push(!stk.pop());			// !를 처리한다.
		if (kd == Minus) stk.push(-stk.pop());			// -를 처리한다.
...
void fncCall_syntax(int fncNbr)					// 함수 호출 검사
{
	...
	if (code.kind != ')') {								// 인수가 있다
		for (;; code = nextCode()) {
			(void)get_expression(); ++argCt;			// 인수식 처리와 인수 갯수
			if (code.kind != ',') break;				// , 이면 인수가 계속된다.
		}
	}
	code = chk_nextCode(code, ')');						// ) 일 것
	if (argCt != Gtable[fncNbr].args)
		err_exit(Gtable[fncNbr].name, " 함수의 인수 갯수가 잘못 되었습니다.");
...
void fncCall(int fncNbr)						// 함수 호출
{
	...
	if (code.kind != ')') {
		for (;; code = nextCode()) {
			expression(); ++argCt;						// 인수식 처리와 인수 갯수
	...
	// 인수 저장 순서 변경
	for (n = 0; n < argCt; n++)
		vc.push_back(stk.pop());						// 뒤에서부터 인수 저장
	for (n = 0; n < argCt; n++) {
		stk.push(vc[n]);
	}
...
int get_memAdrs(const CodeSet& cd)
{
	...
	if (syntaxChk_mode) return adr;						// 구문 검사시

	index = (int) d;
	if (index < 0 || len <= index)
		err_exit(index, " 는 첨자범위 밖입니다. (첨자범위:0-", len-1, ") ");
...
TknKind lookCode(int line)						// line 행의 시작코드
{
